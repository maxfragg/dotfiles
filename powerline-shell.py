...
def add_ssh_segment(powerline, sshflag):
    fg = Color.SSH_FG
    bg = Color.SSH_BG
    if sshflag != "0":
        ssh_marker = u'\u2708'+" "
        powerline.append(Segment(powerline, ssh_marker, fg, bg))
...
def add_host_segment(powerline, cwd):
    ...
    hs = hostname
    bg = 254
    if hostname == "max-x61-xu":
        hs = "x61"
        bg = 214 #orange
    elif hostname == "phenux":
        bg = 76 #green
...
def add_root_indicator(powerline, error):
    ...
    st = powerline.root_indicator
    if int(error) != 0:
        fg = Color.CMD_FAILED_FG
        bg = Color.CMD_FAILED_BG
        st += '(' + error + ')'
    powerline.append(Segment(powerline, st, fg, bg))
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--sshflag', action='store',default="")
